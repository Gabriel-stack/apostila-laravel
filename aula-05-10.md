...
O comando acima vai perguntar se você quer sobrescrever os arquivos de autenticação. Digite `yes` e aperte enter para sobrescrever os arquivos de autenticação. **Digite**  `no` quando for perguntado se você quer sobrescrever o arquivo `home.blade.php` e o arquivo `HomeController.php`. **Não sobrescreva** esses arquivos. 
